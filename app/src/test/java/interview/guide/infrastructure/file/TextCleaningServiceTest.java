...
class TextCleaningServiceTest {
    ...
    class CleanTextTests {
        ...
        @Test
        @DisplayName("兼容区字符应归一化为普通字符")
        void testNormalizeCompatibilityIdeographs() {
            String input = "Redis ⾯试⾼频题与ＡＯＦ持久化";

            String result = textCleaningService.cleanText(input);

            assertEquals("Redis 面试高频题与AOF持久化", result);
        }
    }
    ...
    @Nested
    @DisplayName("兼容字符检测测试")
    class CompatibilityDetectionTests {

        @Test
        @DisplayName("应检测出 PDF 常见兼容字符")
        void testContainsCompatibilityCharacters() {
            assertTrue(textCleaningService.containsCompatibilityCharacters("⾯试⾼频题"));
            assertFalse(textCleaningService.containsCompatibilityCharacters("面试高频题"));
        }
    }
}
